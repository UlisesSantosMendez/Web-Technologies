...
<body>
    ...
    <section name="area-desplegar">
        ...
        <?php
        $administradores= mysqli_query($conexion,"SELECT id_usuario, nombre, usuario, tipo_usuario, correo FROM `usuarios` WHERE tipo_usuario='Administrador'");
        $res_admin= mysqli_num_rows($administradores);
        if($res_admin > 0){
            while($data = mysqli_fetch_array($administradores)){
        ?>
        <tr>
