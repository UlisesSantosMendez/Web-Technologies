...
<body>
    ...
    <section name="area-desplegar">
        ...
        <?php
        $alumnos= mysqli_query($conexion,"SELECT id_usuario, nombre, usuario, tipo_usuario, correo FROM `usuarios` WHERE tipo_usuario='Alumno'");
        $res_alum= mysqli_num_rows($alumnos);
        if($res_alum > 0){
            while($data = mysqli_fetch_array($alumnos)){
        ?>
        <tr>
