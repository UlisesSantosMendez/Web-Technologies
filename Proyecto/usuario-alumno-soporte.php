...
<body>
    ...
    <img class="logo"
        ...
        <ul class="nav">
                ...
                <li><a href="/project/usuario-alumno-soporte.php?id=<?php echo $usuario?>" style="background-color: blue;">Soporte</a></li>
                <li><a href="/project/usuario-alumno-ay.php?id=<?php echo $usuario?>">Ayuda</a></li>
            </ul>
        </nav>
    <section name="area-desplegar">
        <h2>Agendar cita para asesoría</h2><br>
        <form action="/project/add-cita.php" method="POST">
            <label for="">Fecha y hora de la cita:</label>
            <input type="date" name="fecha_cita"><br><br>
            <div style="color: red; font-size:smaller; margin-bottom:2rem"><?php
            if(empty($_POST['fecha_cita']))
            echo  "*Ingrese un fecha válida."
            ?></div>
            <button type="submit" class="enviar-datos" onclick="alert('Datos enviados satisfactoriamente...')" name="guardar-datos">Verificar disponibilidad</button>
        </form>
